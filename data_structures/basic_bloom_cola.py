...
class BasicBloomCola(WriteOptimizedDS):
    ...
    def __init__(self, disk_filepath, block_size, n_blocks, n_input_data, growth_factor=2, pointer_density=0.1):
        ...
        self.big_bloom_filter = BloomFilter(capacity=self.n_input_data, error_rate=ERROR_RATE)
        ...
        for level_size in self.level_sizes:
            self.bloom_filters.append(BloomFilter(capacity=level_size, error_rate=ERROR_RATE))
    ...
    def insert(self, item):
        ...
        for i in range(self.n_levels):
            ...
            if merge_size > level_size:  # it will be full
                ...
                self.bloom_filters[i] = BloomFilter(capacity=self.level_sizes[i], error_rate=ERROR_RATE)
    ...
    def _search(self, item):
        if item not in self.big_bloom_filter:  # check bloom filter first.
        ...
        for i in range(n_search_levels):
            ...
            if item not in self.bloom_filters[i]:
                continue  # do not bother searching
